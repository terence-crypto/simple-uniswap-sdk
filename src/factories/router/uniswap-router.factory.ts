...
export class UniswapRouterFactory {
  ...
  public async getAllPossibleRoutes(): Promise<AllPossibleRoutes> {
    if (
      this._allPossibleRoutes.v2.length > 0 ||
      this._allPossibleRoutes.v3.length > 0
    ) {
      return this._allPossibleRoutes;
    }
    ...
    this._allPossibleRoutes = allPossibleRoutes;
  ...
  private get CustomTokensForConnectedNetwork() {
    if (
      this._settings.customNetwork &&
      this._settings.customNetwork.customBaseTokens
    ) {
