...
export class UniswapRouterFactory {
  ...
  public async getAllPossibleRoutes(): Promise<AllPossibleRoutes> {
    ...
    if (!this._settings.disableMultihops) {
      findPairs = [
        ...
        this.mainCurrenciesPairsForCustomTokens,
    ...
    if (this._settings.uniswapVersions.includes(UniswapVersion.v3)) {
      ...
      for (let i = 0; i < results.callsReturnContext.length; i++) {
        if (results.callsReturnContext[i].returnValues[0] !== "0x0000000000000000000000000000000000000000") {
  ...
  public async getAllPossibleRoutesWithQuotes(
    ...
    if (this._settings.uniswapVersions.includes(UniswapVersion.v2)) {
      ...
      for (let i = 0; i < routes.v2.length; i++) {
        ...
        contractCallContext[0].calls.push({
          reference: `route${i}`,
          methodName: direction === TradeDirection.input ? "getAmountsOut" : "getAmountsIn",
    ...
    if (this._settings.uniswapVersions.includes(UniswapVersion.v3)) {
      ...
      for (let i = 0; i < routes.v3.length; i++) {
        ...
        contractCallContext[this._settings.uniswapVersions.includes(UniswapVersion.v2) ? 1 : 0].calls.push({
          reference: `route${i}`,
          methodName: direction === TradeDirection.input ? "quoteExactInputSingle" : "quoteExactOutputSingle",
          methodParameters: [
            routeCombo[0],
            routeCombo[1],
  ...
  public async findBestRoute(amountToTrade: BigNumber, direction: TradeDirection): Promise<BestRouteQuotes> {
    ...
    const allowanceAndBalances = await this.hasEnoughAllowanceAndBalance(amountToTrade, allRoutes[0], direction);
  ...
  private generateTradeDataForV3Input(
    ...
    const isNativeReceivingNativeEth = isNativeEth(this._toToken.contractAddress);
    const params: ExactInputSingleRequest = {
      ...
      recipient:
        isNativeReceivingNativeEth === true ? "0x0000000000000000000000000000000000000000" : this._ethereumAddress,
      deadline,
    ...
    if (isNativeEth(this._toToken.contractAddress)) {
      multicallData.push(
        this._uniswapRouterContractFactoryV3.unwrapWETH9(hexlify(tokenAmountMin), this._ethereumAddress)
      );
    }
   ...
   * @param data The data
   */
  private buildUpTransactionErc20(uniswapVersion: UniswapVersion, data: string): Transaction {
    return {
      to:
   ...
   * @param data The data
   */
  private buildUpTransactionEth(uniswapVersion: UniswapVersion, ethValue: BigNumber, data: string): Transaction {
    return {
      to:
        uniswapVersion === UniswapVersion.v2
          ? uniswapContracts.v2.getRouterAddress(this._settings.cloneUniswapContractDetails)
    ...
    switch (this.tradePath()) {
      case TradePath.ethToErc20:
        const result = await this.hasGotEnoughBalanceEth(
          direction === TradeDirection.input ? amountToTrade.toFixed() : bestRouteQuote.expectedConvertQuote
        );
  ...
  private async filterWithTransactionFees(
    ...
    if (this._settings.gasSettings && !this._settings.disableMultihops) {
      ...
      const fiatPrices = await this._coinGecko.getCoinGeckoFiatPrices([this._toToken.contractAddress, ethContract]);
      ...
      if (toUsdValue && ethUsdValue) {
        const bestRouteQuoteHops = this.getBestRouteQuotesHops(allRoutes, enoughAllowanceV2, enoughAllowanceV3);
        ...
        for (let i = 0; i < bestRouteQuoteHops.length; i++) {
          ...
          const expectedConvertQuoteFiatPrice = new BigNumber(route.expectedConvertQuote).times(toUsdValue);

          const txFee = formatEther(
            new BigNumber((await this._ethersProvider.provider.estimateGas(route.transaction)).toHexString()).times(
              gasPrice
            )
          ).times(ethUsdValue);
        ...
        if (bestRoute) {
          const routeIndex = allRoutes.findIndex(
            (r) =>
              r.expectedConvertQuote === bestRoute!.routeQuote.expectedConvertQuote &&
  ...
  private getBestRouteQuotesHops(
    ...
    for (let i = 0; i < allRoutes.length; i++) {
      ...
      if (route.uniswapVersion === UniswapVersion.v2 ? enoughAllowanceV2 : enoughAllowanceV3) {
            ...
            case TradePath.ethToErc20:
              result.push(
                this.buildRouteQuoteForEthToErc20(
                  ...
                  direction,
                  contractCallReturnContext.originalContractCallContext.reference as UniswapVersion
                )
              );
              break;
            case TradePath.erc20ToEth:
  ...
  private buildRouteQuoteForErc20ToErc20(
    ...
    const convertQuoteUnformatted = this.getConvertQuoteUnformatted(callReturnContext, direction, uniswapVersion);
    ...
    const expectedConvertQuoteOrTokenAmountInMaxWithSlippage =
      this.getExpectedConvertQuoteOrTokenAmountInMaxWithSlippage(expectedConvertQuote, direction, uniswapVersion);
    ...
    switch (uniswapVersion) {
      ...
      case UniswapVersion.v3:
        return {
          ...
          routePathArray: [this._fromToken.contractAddress, this._toToken.contractAddress],
          uniswapVersion,
          liquidityProviderFee: routeContext.liquidityProviderFee,
          quoteDirection: direction,
        };
  ...
  private buildRouteQuoteForEthToErc20(
    ...
    const transaction = this.buildUpTransactionEth(
      uniswapVersion,
      direction === TradeDirection.input ? amountToTrade : new BigNumber(expectedConvertQuote),
      data
    );
      ...
      case UniswapVersion.v3:
        return {
          ...
          routePathArrayTokenMap: [
            turnTokenIntoEthForResponse(this._fromToken, this._settings?.customNetwork?.nativeCurrency),
            this._toToken,
          ],
          routeText: `${turnTokenIntoEthForResponse(this._fromToken, this._settings?.customNetwork?.nativeCurrency).symbol
    ...
    switch (uniswapVersion) {
      ...
      case UniswapVersion.v3:
        return {
          ...
          routePathArrayTokenMap: [
            this._fromToken,
            turnTokenIntoEthForResponse(this._toToken, this._settings?.customNetwork?.nativeCurrency),
          ],
          routeText: `${this._fromToken.symbol} > ${turnTokenIntoEthForResponse(this._toToken, this._settings?.customNetwork?.nativeCurrency).symbol
            }`,
          routePathArray: [this._fromToken.contractAddress, this._toToken.contractAddress],
          uniswapVersion,
          liquidityProviderFee: routeContext.liquidityProviderFee,
  ...
  private getExpectedConvertQuoteOrTokenAmountInMaxWithSlippage(
    ...
    _uniswapVersion: UniswapVersion
  ): string {
    ...
    if (tradeDirection === TradeDirection.output) {
      return new BigNumber(expectedConvertQuote)
        .plus(new BigNumber(expectedConvertQuote).times(this._settings.slippage).toFixed(decimals))
        .toFixed(decimals);
    }
  ...
  private get allMainTokens(): Token[] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      for (const token of this.CustomTokensForConnectedNetwork) {
        tokens.push(token)
      }
  ...
  private get mainCurrenciesPairsForFromToken(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      for (const token of this.CustomTokensForConnectedNetwork) {
        pairs.push([this._fromToken, token])
      }

      if (!isNativeEth(this._fromToken.contractAddress) && !isNativeEth(this._toToken.contractAddress)) {
  ...
  private get mainCurrenciesPairsForToToken(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      for (const token of this.CustomTokensForConnectedNetwork) {
        pairs.push([token, this._toToken])
      }

      if (!isNativeEth(this._toToken.contractAddress) && !isNativeEth(this._toToken.contractAddress)) {
    ...
    const pairs: Token[][] = [[this.WETHTokenForConnectedNetwork, this._toToken]];
  ...
  private get mainCurrenciesPairsForUSDT(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      for (const token of this.CustomTokensForConnectedNetwork) {
        pairs.push([this.USDTTokenForConnectedNetwork, token])
      }

      if (!isNativeEth(this._fromToken.contractAddress) && !isNativeEth(this._toToken.contractAddress)) {
        pairs.push([this.USDTTokenForConnectedNetwork, this.WETHTokenForConnectedNetwork]);
      }
  ...
  private get mainCurrenciesPairsForCOMP(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      if (!isNativeEth(this._fromToken.contractAddress) && !isNativeEth(this._toToken.contractAddress)) {
        pairs.push([this.COMPTokenForConnectedNetwork, this.WETHTokenForConnectedNetwork]);
      }
  ...
  private get mainCurrenciesPairsForDAI(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      if (!isNativeEth(this._fromToken.contractAddress) && !isNativeEth(this._toToken.contractAddress)) {
        pairs.push([this.DAITokenForConnectedNetwork, this.WETHTokenForConnectedNetwork]);
      }
  ...
  private get mainCurrenciesPairsForUSDC(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      if (!isNativeEth(this._fromToken.contractAddress) && !isNativeEth(this._toToken.contractAddress)) {
        pairs.push([this.USDCTokenForConnectedNetwork, this.WETHTokenForConnectedNetwork]);
      }
  ...
  private get mainCurrenciesPairsForWETH(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      ...
      for (const token of this.CustomTokensForConnectedNetwork) {
        tokens.push([this.WETHTokenForConnectedNetwork, token])
      }
  ...
  private get mainCurrenciesPairsForCustomTokens(): Token[][] {
    if (this._ethersProvider.provider.network.chainId === ChainId.MAINNET || this._settings.customNetwork) {
      const pairs = []
      for (const token of this.CustomTokensForConnectedNetwork) {
        pairs.push(
          [token, this.USDTTokenForConnectedNetwork],
          [token, this.COMPTokenForConnectedNetwork],
          [token, this.DAITokenForConnectedNetwork],
          [token, this.USDCTokenForConnectedNetwork],
          [token, this.WBTCTokenForConnectedNetwork]
        )

        if (!isNativeEth(this._fromToken.contractAddress) && !isNativeEth(this._toToken.contractAddress)) {
          pairs.push([token, this.WETHTokenForConnectedNetwork]);
        }
      }
  ...
  private get CustomTokensForConnectedNetwork() {
    if (this._settings.customNetwork && this._settings.customNetwork.customBaseTokens) {
      return Object.values(this._settings.customNetwork.customBaseTokens);
    }
